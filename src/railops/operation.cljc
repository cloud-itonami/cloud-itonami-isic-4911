(ns railops.operation
  "OperationActor -- one operations-coordination request = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Rail-Operations-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Rail Safety
  ...
  One graph run = one operations-coordination request (intake ->
  ...
  `interrupt-before #{:request-approval}` pauses the actor and hands
  the decision to a human operations-coordination desk operator. The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:flag-passenger-safety-concern` ALWAYS reaches this
  node when the governor is clean -- see `railops.phase`/
  `railops.governor`."
...
(defn- commit-record [request _context proposal]
  {:effect  (:effect proposal)
   ...
   :payload (or (:value proposal) {})})

(defn build
  "Compiles an OperationActor graph bound to `store` (any
  `railops.store/Store`).
  opts:
    :advisor      -- a `railops.railopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (railopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Rail-Operations-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (railopsllm/-advise advisor store request)]
            {:proposal p :audit [(railopsllm/trace request p)]})))

      ;; Rail Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :safety-concern
                                          :else :low-confidence))
                        :phase ph
